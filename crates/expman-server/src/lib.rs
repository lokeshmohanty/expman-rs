...
mod server {
    ...
    pub use crate::state::ServerConfig;
                ...
                for name in run_names {
                    ...
                    if meta.vectors.is_none() || meta.vectors.as_ref().unwrap().is_empty() {
                        if let Ok(scalars) = storage::read_latest_scalar_metrics(&vectors_path) {
                            if !scalars.is_empty() {
                                let converted: HashMap<String, expman::models::MetricValue> =
                                    scalars
                                        .into_iter()
    ...
    async fn get_experiment_stats(
        ...
        for run_name in &runs {
            ...
            let last_metrics = storage::read_latest_scalar_metrics(&dir.join("vectors.parquet"))
                .unwrap_or_default();
...
// Re-export public items from the server module for native builds
#[cfg(not(target_arch = "wasm32"))]
